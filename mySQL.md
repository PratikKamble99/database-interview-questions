...
3. **Correlated sun-query**
    ...
    Sub-Queries allowed in 1. SELECT : sub-query should return only one column and one row 2. FROM 3. WHERE 4. HAVING
...
NOTES: 1) When you want to use distinct in functions use inside function
...
- lag(column_name, count_of_how_many_rows_behind): return prev record, EG. lag(num, 3) -> 3 rows behind
...
- lead(column_name, count_of_how_many_rows_after): return next record, EG. lag(num, 3) -> 3 rows after
...
## 43. Boolean Expression Evaluates to 0 OR 1
...
Databases where this works (MOST IMPORTANT)
...
**FUNCTIONS / PLACES where this WORKS as 1 / 0**
...
4. GROUP BY (advanced)
    ```mysql
    SELECT status != 'completed' AS flag, COUNT(*)
    FROM Trips
    GROUP BY flag;
    ```
